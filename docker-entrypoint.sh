...
EOF
    ...
    # Setup database and user permissions for both root and canteen_user
    mysql -e "
        CREATE DATABASE IF NOT EXISTS \`$DB_NAME\` CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci;
        GRANT ALL PRIVILEGES ON *.* TO 'root'@'localhost' WITH GRANT OPTION;
        GRANT ALL PRIVILEGES ON *.* TO 'root'@'127.0.0.1' WITH GRANT OPTION;
        GRANT ALL PRIVILEGES ON *.* TO 'root'@'%' IDENTIFIED BY '' WITH GRANT OPTION;
        CREATE USER IF NOT EXISTS 'canteen_user'@'%' IDENTIFIED BY '';
        GRANT ALL PRIVILEGES ON *.* TO 'canteen_user'@'%' WITH GRANT OPTION;
        CREATE USER IF NOT EXISTS 'canteen_user'@'localhost' IDENTIFIED BY '';
